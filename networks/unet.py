...
class UNet(nn.Module):
    def __init__(
        ...
        self.encs = nn.ModuleList(
            [
                EncBlock(channels[i], channels[i + 1], dropout)
        ...
        self.decs = nn.ModuleList(
            [
                DecBlock(channels[i], channels[i - 1], dropout)
...
class AttentionBlock(nn.Module):
    def __init__(self, input_dim, output_shape, hidden_dim=64):
        ...
        assert (
            len(output_shape) == 2
        ), "output_shape should have the form L x W of the desired output, excluding channels"
    ...
    def forward(self, date, x):
        ...
        attention_mask = attention_mask.view(
            -1, 1, self.output_shape[0], self.output_shape[1]
        )
...
class ExtraUNet(nn.Module):
    def __init__(
        ...
        self.encoder = nn.ModuleList(
            [
                EncBlock(
                    in_features, 64, use_batchnorm=use_batchnorm
                ),  # image shape after pooling: h/2 x w/2
        ...
        if use_attention:
            self.attention_blocks = nn.ModuleList(
                [
                    ...
                    AttentionBlock(
                        input_dim=time_features, output_shape=image_shape // 2
                    ),
                    AttentionBlock(
                        input_dim=time_features, output_shape=image_shape // 4
                    ),
                    AttentionBlock(
                        input_dim=time_features, output_shape=image_shape // 8
                    ),
                ]
            )
        else:
