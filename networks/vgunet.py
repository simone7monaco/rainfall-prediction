...
class SpatialGCN(nn.Module):
    ...
    def forward(self, x):
        node_k = self.node_k(
            x
        )  # x#copy.deepcopy(x)#F.normalize(x,p=1,dim=-1)   #####nosym better, softmax better,only one gcn better
        ...
        AVW = F.relu(
            self.bn1(self.conv_wgl(AV).transpose(1, 2)).transpose(1, 2)
        )  ###AXW b n C
        AVW = F.dropout(AVW)
        # add one more layer
...
class VGUNet(nn.Module):
    def __init__(
        self, in_ch=2, out_ch=2, base_nc=64, fix_grad=True, layers_mults=(1, 2, 4)
    ):
        ...
        self.sgcns = nn.ModuleList(
            [nn.Identity()]
        )  # the last block do not use spatial gcn
        ...
        self.ups = nn.ModuleList(
            [
                nn.ConvTranspose2d(
                    self.sgcns[i].plane,
                    self.enc_convs[-(i + 1)].out_channels,
                    2,
                    stride=2,
                    padding=0,
                    bias=False,
        ...
        self.dec_convs = nn.ModuleList(
            [
                DoubleConv(
                    self.enc_convs[-(i + 1)].out_channels * 2,
                    self.enc_convs[-(i + 1)].out_channels,
                )
                for i in range(len(self.enc_convs))
            ]
